...
# 4. 解压
sudo tar -vzxf gost_3.0.0_linux_amd64.tar.gz
...
# 7. 替换 ExecStart 里的变量
sudo sed -i "s|\\\$USER|$USER|g" /etc/systemd/system/gost.service
sudo sed -i "s|\\\$PASS|$PASS|g" /etc/systemd/system/gost.service
sudo sed -i "s|\\\$PORT|$PORT|g" /etc/systemd/system/gost.service
